...
MONGODB_URI=mongodb://<mongodb-username>:<mongodb-passward>@mongo:27017/trend_agent_db?authSource=admin
MONGODB_DB=trend_agent_db
```
...
#### 1. 렌딩페이지, 사용자 등록 / 로그인 (간단하게 구현)

![렌딩페이지](images/1_Landing_0.png)
<br>렌딩페이지 ([https://chwlabs.dev](https://chwlabs.dev))

![유저등록](images/1_Landing_1_registry.png)
<br>회원가입(단순 유저 CREATE)

![로그인](images/1_Landing_2_login.png)
<br>로그인

#### 2. 메인페이지 / 템플릿 질문 요청 -> 대화창 진입 -> 답변 완료 -> 새 대화 클릭

![메인페이지](images/2_Threads_0.png)
<br>메인페이지 (/chat)

![템플릿 질문 중 첫번째 클릭](images/2_Threads_1_temp_Q.png)
<br>템플릿 질문 중 첫번째 클릭 (AI 에이전트 시장 블록)

![답변 생성중](images/2_Threads_2_temp_n.png)
<br>답변 생성중

![답변 생성 완료](images/2_Threads_3_temp_A.png)
<br>답변 생성 완료

![새 대화 버튼 클릭](images/2_Threads_4_newChat.png)
<br>좌측 상단 "새 대화" 버튼 클릭 -> 메인페이지로 이동

#### 3. 메인페이지 / 임의 질문 -> 대화창 진입 -> 답변 생성 -> 답변 완료

![메인페이지, 임의질문작성, 전송](images/2_Threads_5_custom_Q.png)
<br>메인페이지에서 하단 검색바에 임의질문작성 -> 전송

![제대로된 질문 다시 전송](images/3_Chat_1_custom_A.png)
<br>답변 생성 후, 새로운 질문 전송

![답변 생성](images/3_Chat_2_custom_A2.png)
<br>답변 생성
